# coding:utf-8
from . import  api
from ihome.modles import  Area
from  flask import  current_app,jsonify,g,request,session
from ihome.utils.response_code import RET
from ihome import redis_store,constants,db
import json
from ihome.utils.commons import log_required
from ihome.modles import House,Facility,HouseImage,User,Order
...
def save_house_info():
    ...
    if facility_id_list:
        ...
        try:
            facility_id_list = Facility.query.filter(Facility.id.in_(facility_id_list)).all()
        except Exception as e:
            current_app.logger.error(e)
            return facility_id_list(errno=RET.DBERR,errmsg="数据库异常")


        #为房屋添加设施信息
        if facility_id_list:
            house.facilities = facility_id_list
    try:
...
def save_house_image():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(error = RET.THIRDERR,errmsg="保存房屋信息失败")
...
def get_house_list():
    """获取房屋列表信息"""
    #获取参数
    start_date_str = request.args.get("sd","")#想要查询的起始日期
    end_date_str = request.args.get("ed","")#想要查询的结束日期日期
    ...
    if resp_json:
        #从缓存中拿到了数据
        return resp_json,200,{"Content-Type":"applicatiaon/json"}
    ...
    if conflict_orders_li:
        ...
        #添加条件,查询不冲突的房屋
        filter_params.append(not House.idin_(conflict_house_id_li))

    #排序
    if sort_key == "booking":
        hou_query = House.query.filter(*filter_params).order_by(House.order_count.desc())
    elif sort_key == "price-inc":
        house_query = House.query.filter(*filter_params).order_by(House.order_count.asc())
    ...
    if page <= total_page:
        #用redis的哈希类型保存分页数据
        redis_key = "house_%s_%s_%s_%s" %(start_date_str,end_date_str,area_id,sort_key)
        try:
            #使用redi中的事物
